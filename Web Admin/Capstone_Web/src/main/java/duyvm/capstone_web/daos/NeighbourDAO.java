...
public class NeighbourDAO {
	...
	public BuildingDTO attachNeighbour(BuildingDTO buildingDTO, String locationId, NeighbourDTO neighbourInfo) {
		...
		NeighbourDTO neighbourDTO = new NeighbourDTO(neighbourInfo.getId(), neighbourInfo.getName(), neighbourInfo.getOrientation(), neighbourInfo.getDistance(), true);
		...
		for (int i = 0; i < buildingDTO.getListFloor().size(); i++) {
			for (int j = 0; j < buildingDTO.getListFloor().get(i).getListLocation().size(); j++) {
				if (buildingDTO.getListFloor().get(i).getListLocation().get(j).getId().equals(locationId)) {
					listLocationBeside = buildingDTO.getListFloor().get(i).getListLocation().get(j).getListLocationBeside();
					listLocationBeside.add(neighbourDTO);
					buildingDTO.getListFloor().get(i).getListLocation().get(j).setListLocationBeside(listLocationBeside);
					break;
				}
			}
		}
	...
	public BuildingDTO changeNeighbourStatus(BuildingDTO buildingDTO, String rootLocationId, String neighbourLocationId, boolean status) {

		for (int i = 0; i < buildingDTO.getListFloor().size(); i++) {
			for (int j = 0; j < buildingDTO.getListFloor().get(i).getListLocation().size(); j++) {
				if (rootLocationId.equals(buildingDTO.getListFloor().get(i).getListLocation().get(j).getId())) {
					for (int k = 0; k < buildingDTO.getListFloor().get(i).getListLocation().get(j).getListLocationBeside().size(); k++) {
						if (neighbourLocationId.equals(buildingDTO.getListFloor().get(i).getListLocation().get(j).getListLocationBeside().get(k).getId())) {
							buildingDTO.getListFloor().get(i).getListLocation().get(j).getListLocationBeside().get(k).setActive(status);
							break;
						}
					}
				}
